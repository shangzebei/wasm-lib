...
func (m *LLVMManger) Init(vm *exec.VirtualMachine, vMem types.VMemory) {
	wasm.AddFunc(m, "Sbrk")
	wasm.AddFunc(m, "Brk")
	m.vm = vm
	wasm.SetVMemory(vMem)
	m.GLOBAL_BASE = 1024
	m.TOTAL_STACK = util.CheckIFElse(HEAP_BASE(vm)-DATA_END(vm), 5242880)
	m.TOTAL_MEMORY = m.GetTotalMemory()
	m.STATICTOP = m.GLOBAL_BASE + util.CheckIFElse(DATA_END(vm), 24400)
...
func (m *LLVMManger) CheckUnflushedContent() {
	FFLUSH(m.vm, 0)
...
func (m *LLVMManger) enlargeMemory() bool {
	return m.abortOnCannotGrowMemory()
}

func (m *LLVMManger) StaticAlloc(size int64) int64 {
	var ret = m.STATICTOP
	m.STATICTOP = m.STATICTOP + size + 15&-16
	return ret
}

func (m *LLVMManger) AlignMemory(size int64) int64 {
	factor := STACK_ALIGN
	var ret = int64(math.Ceil(float64(size/factor))) * factor
	return ret
}

func (m *LLVMManger) AllocateUTF8OnStack(str string) int64 {
	size := len([]byte(str)) + 1
...
//int sbrk(void *addr);
func (v *LLVMManger) Sbrk(increment int) int32 {
...
func (v *LLVMManger) Brk(newDynamicTop int) int {
